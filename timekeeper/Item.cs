...
namespace Timekeeper
{
    class Item
    {
        ...
        protected int secondsOne = 0;
        
        protected void _load(int id)
        {
            // fetch row from db
            ...
            this.is_folder = Convert.ToBoolean(row["is_folder"]);
            this.is_hidden = Convert.ToBoolean(row["is_hidden"]);
            this.is_deleted = Convert.ToBoolean(row["is_deleted"]);
            if (row["timestamp_c"].Length > 0) {
                this.timestamp_c = DateTime.Parse(row["timestamp_c"]);
            }
            if (row["timestamp_m"].Length > 0)
            {
                this.timestamp_m = DateTime.Parse(row["timestamp_m"]);
            }
            if (row.ContainsKey("project_id__last")) {
                if (row["project_id__last"].Length > 0) {
                    project_id__last = Convert.ToInt32(row["project_id__last"]);
                } else {
                    project_id__last = -1;
                }
            }
        ...
        protected void getSeconds(int offset)
        {
        ...
        public int countTreeTime(int id, string fromDate, string toDate)
        {
            // get self
            string query = String.Format(@"
                select sum(seconds) as seconds
        ...
        public int create()
        {
            ...
            row["parent_id"] = this.parent_id.ToString();
        ...
        public TimeSpan elapsedTodayAll(int seconds)
        {
            DateTime currentTime = DateTime.Now;
        ...
        public string elapsedTodayStatic()
        {
            ...
            return Common.FormatTimeSpan(ts);
        }

        public string elapsedTodayAllStatic(int seconds)
        {
            DateTime midnight = DateTime.Parse("00:00:00");
